...
scene.add(building.group, ...rackings.map(r => r.group), ...shelvings.map(s => s.group), conveyor.group, packing.group, slam.group, rebin.group, outbound.group)
const colliders = [...building.colliders, ...rackings.flatMap(r => r.colliders), ...shelvings.flatMap(s => s.colliders), ...conveyor.colliders, ...packing.colliders, ...slam.colliders, ...rebin.colliders, ...outbound.colliders]
...
const picker = new Picker(canvas, [...rackings.map(r => r.faceVolumes), ...rackings.map(r => r.volumes), ...shelvings.map(s => s.bins), building.dockVolumes, packing.volumes, slam.volumes, rebin.volumes, outbound.volumes, building.zoneVolumes], () => rig.camera)
...
function walkTo(e: Entity) {
  const off = e.kind === 'bay' || e.kind === 'face' ? RACK.aisle / 2 + RACK.frameDepth / 2 : e.kind === 'bin' ? SHELF.aisle / 2 + SHELF.unitD / 2 + 0.2 : e.kind === 'dock' ? 4 : e.kind === 'station' ? 1.1 : e.kind === 'wall' ? 1.2 : e.kind === 'slam' ? 3 : e.kind === 'lane' ? (e.role === 'sort' ? 2 : 10) : 0
...
const standoff = (e: Entity) => e.kind === 'bin' ? 0.7 : e.kind === 'face' ? 2.4 : e.kind === 'dock' ? 12 : e.kind === 'station' ? 4.5 : e.kind === 'wall' ? 5 : e.kind === 'slam' ? 10 : e.kind === 'lane' ? (e.role === 'sort' ? 5 : 16) : e.kind === 'zone' ? THREE.MathUtils.clamp(Math.max(e.size[0], e.size[2]) * 0.6, 10, 42) : undefined
